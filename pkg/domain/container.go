...
const (
	...
	// ContainerOpenSent State represent the first phase of the handshake sending the OpenFrame to the client.
	ContainerOpenSent ContainerState = "OPEN_SENT"

	// ContainerOpenRcvd State represent the confirmation of the received OpenFrame from the client.
	ContainerOpenRcvd ContainerState = "OPEN_RCVD"

	// ContainerConnected State represent the validation and connection from the client
	// to the broker and fully assigned to his container.
	ContainerConnected ContainerState = "CLIENT_CON"

	// ContainerReserved State represent the state when a client shutdown
	// and the container have no more clients reserve the container while clients are rebooting.
	ContainerReserved ContainerState = "RSRVD"
